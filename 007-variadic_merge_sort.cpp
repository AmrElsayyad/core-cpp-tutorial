...
template <typename Ints1> struct SortedArrayHelper;

template <int... Vals> struct SortedArrayHelper<Ints<Vals...>> {
  static const array<int, sizeof...(Vals)> arr;
};

template <int... Vals>
const array<int, sizeof...(Vals)> SortedArrayHelper<Ints<Vals...>>::arr = {
    {Vals...}};

template <int... Vals>
struct SortedArray : public SortedArrayHelper<typename Sort<Vals...>::type> {};

int main() {
  typedef SortedArray<11, 13, 5, 79, 73, 47, 31, 89, 2, 97, 41, 11, 61, 19, 29,
                      29, 59, 67, 89, 53, 2, 37, 43, 17, 47, 89, 79, 23, 61, 83,
                      7, 83, 11, 3, 13, 3, 11, 59, 71, 37, 29, 19, 83>
      meow;

  for (const auto &i : meow::arr) {
    cout << i << " ";
  }
